...
public class LoansController {
    ...
    private static final AtomicInteger retryCounter = new AtomicInteger(-1);

    @GetMapping(value = "/contact-loans-support-team")
    public ResponseEntity<ContactInfoLoansDevTeam> contactLoanDevTeam(){
        int attempt = retryCounter.incrementAndGet();
        log.error("🔥 Retry attempt {} → invoked contact-info of loans", attempt);
